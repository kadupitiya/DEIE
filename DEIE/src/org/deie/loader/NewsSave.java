...
public class NewsSave {
	...
	private final String UPLOAD_DIRECTORY_News = "assets/images/news";
	...
	public Message addNews(HttpServletRequest request) {
		...
		String uploadPath = request.getServletContext().getRealPath("") + File.separator + UPLOAD_DIRECTORY_News;
		...
		if (isMultipart) {
			...
			try {
				...
				for (FileItem item : multiparts) {
					if (!item.isFormField()) {
						...
						if (item.getFieldName().equals("noticeTitle")) {
							...
							spID = item.getString();
							current.setNewsId(Integer.parseInt(spID));
							index = spID;
						}
				...
				String statement1 = "INSERT INTO News (`newsId`, `title`, `description`, `imageName`, `imageCount`) VALUES ('"
	...
	public Message getNewsID(HttpServletRequest request) {
		...
		try {
			...
			String statement = "SELECT MAX(newsId)+1 as newsId FROM News";
	...
	public Message getNewsIDList(HttpServletRequest request) {
		...
		try {
			...
			String statement = "SELECT DISTINCT newsId FROM News ORDER BY newsId";
	...
	public Message setDisableSpecialNews(String index) {
		...
		try {
			...
			String statement2 = "SELECT * FROM News WHERE newsId='" + index + "'";
			...
			String statement = "UPDATE News SET isDisable='"
	...
	public Message updateNews(HttpServletRequest request) {
		...
		if (isMultipart) {
			...
			try {
				...
				if(val.equals("editButton")){
					...
					String statement1 = "UPDATE News SET title='"
